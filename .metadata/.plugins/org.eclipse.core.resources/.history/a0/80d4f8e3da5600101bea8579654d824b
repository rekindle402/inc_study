...
//클라이언트가 전송한 글쓰리 폼의 파라미터들을 받아, db에 insert 시키는 서블릿
public class RegistServlet extends HttpServlet{
	...
	// 유저가 보게될 디자인과 관련없으므로 서비를릿으로 선택함
...
//		response.setCharacterEncoding("utf-8"); // 응답시 컨텐츠에 문자 인코딩 지정
		...
		try {
			...
			// 클라이언트가 받게될 응답 정보에 스크립트 문자열을 채워넣기
			// Tomcat과 같은 웹컨테이너가, 이 메서드의 닫는 괄호를 만나면. 그 시점부터 나선다
			// 응답 객체인 Response에 들어있는 출력 스트림에 누적되어 있는 문자열을 이용하여, HTML 컨텐츠 생성
			// 하기 떄문에...
			out.print("<script>");
